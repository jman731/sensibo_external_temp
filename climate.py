...
async def async_setup_entry(
    ...
    external_sensor_entity_id = entry.data.get('external_temperature_sensor')

    entities = [
        SensiboClimate(coordinator, device_id, external_sensor_entity_id)
...
class SensiboClimate(SensiboDeviceBaseEntity, ClimateEntity):
    ...
    def __init__(
        self, coordinator: SensiboDataUpdateCoordinator, device_id: str, external_sensor_entity_id
    ) -> None:
        """Initiate Sensibo Climate."""
        super().__init__(coordinator, device_id)
        self.external_sensor_entity_id = external_sensor_entity_id
    ...
    def current_temperature(self) -> float | None:
        external_sensor = self.hass.states.get(self.external_sensor_entity_id)
        if external_sensor and external_sensor.state not in ('unknown', 'unavailable'):
            return float(external_sensor.state)
